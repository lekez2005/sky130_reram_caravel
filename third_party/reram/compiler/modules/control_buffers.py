...
from base.design import design, METAL3, METAL2, METAL1, NIMP, PWELL, TAP_ACTIVE, PIMP, NWELL
...
class ControlBuffers(design, ABC):
    ...
    def get_module_connections(self, connection: Union[SchemConnection, str]):
        ...
        # TODO: sky_tapeout: pgate_horizontal
    ...
    def fill_layers(self):
        ...
        for layer in [NIMP, PIMP]:
            insts = [self.top_insts[0]]
            if self.bottom_insts:
                insts.append(min(self.bottom_insts,key=lambda x: x.lx()))
            for inst in insts:
                if len(insts) == 2 and inst == insts[1]:
                    func = min if layer == NIMP else max
                    right_inst = self.bottom_insts[0]
                else:
                    func = max if layer == NIMP else min
                    right_inst = self.top_insts[-1]
                rect = func(inst.get_layer_shapes(layer, recursive=True),
                            key=lambda x: x.cy())
                x_offset = inst.lx()
                self.add_rect(layer, vector(x_offset, rect.by()),
                              height=rect.height,
                              width=right_inst.rx() - x_offset)
                if layer == NIMP:
                    ext = self.well_enclose_active
                    if len(insts) == 2 and inst == insts[1]:
                        top = rect.uy() + 0.84
                        bot = rect.by() - self.well_enclose_active
                    else:
                        top = rect.uy() + self.well_enclose_active
                        bot = top - 0.84
                    self.add_rect(NWELL, vector(x_offset - ext, bot),
                                  height=top-bot,
                                  width=right_inst.rx() - x_offset + 2 * ext)
